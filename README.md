...
- The notebook connects to `SGJobData_copy.duckdb` with `read_only=True` to avoid file-lock conflicts if the original is open in another kernel. Edit `DB_FILE` in cell 2 to switch back to `SGJobData.duckdb`.
